...
export async function testRateLimitHeaderParsing() {
    ...
    console.log(`\n${passed ? "✅" : "❌"} Rate limit parsing test\n`);
...
export async function testRateLimitStatusFormatting() {
    ...
    console.log(`\n${passed ? "✅" : "❌"} Rate limit formatting test\n`);
        ...
        try {
            ...
            if (result.status === 200 && result.data) {
                ...
                if (result.deprecationWarning) {
                    console.log(`   ⚠️  Deprecation: ${result.deprecationWarning}`);
                }
                passed++;
            } else {
...
export async function quickHealthCheck() {
    ...
    try {
        ...
        if (result.deprecationWarning) {
            console.log(`⚠️  Warning: ${result.deprecationWarning}`);
        }
...
if (typeof window !== "undefined") {
    ...
    console.log("ℹ️  API test utilities loaded. Run 'apiTests.runAllTests()' to start.");
}
